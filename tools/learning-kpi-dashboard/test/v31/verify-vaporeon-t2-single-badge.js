...
async function verifyVaporeonT2SingleBadge() {
  const localFile = 'file:///' + path.resolve(__dirname, '../../../../public/kpi-dashboard.html').replace(/\\/g, '/');
  console.log(`\n================================================================================`);
  console.log(`🤖 [Task 3] 實機 DOM 驗證：水伊布 T1 水之波動 (3 SP) ➡️ T2 獨占 1 對 1 連攜測試`);
  console.log(`================================================================================`);
  ...
  const res = await page.evaluate(() => {
    ...
    // Set 3 SP invested so T2 is unlocked, but ONLY T1 Water Pulse (水之波動) is learned
    pkmn.skillTree = {
      atk: { sp: 3, tier: 2 },
      ATK: { sp: 3, tier: 2 },
      spa: { sp: 0, tier: 1 },
  ...
  const hasAquaJetSynergy = res.textAquaJet.includes('🔗 水之波動·波動共鳴');
  const otherMovesClean = !res.textShellBlade.includes('🔗') && !res.textWaterTail.includes('🔗') && !res.textBubbleBeam.includes('🔗') && !res.textRing.includes('🔗') && !res.textProtect.includes('🔗');

  const isPass = hasAquaJetSynergy && otherMovesClean;
  console.log(`  - 水伊布 T2 獨占 1 對 1 連攜 DOM 驗證: ${isPass ? '✅ 100% PASS' : '❌ FAIL'}`);

  await browser.close();

  if (!isPass) throw new Error('水伊布 T2 依然存在多招式重複連攜標籤！');
}

verifyVaporeonT2SingleBadge();
