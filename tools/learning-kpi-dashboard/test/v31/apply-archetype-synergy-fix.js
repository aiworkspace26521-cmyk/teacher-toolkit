...
console.log(`================================================================`);
console.log(`🎯 修復 calculateMoveSynergyV33 中的 learnedNames Key 解析 Bug`);
console.log(`================================================================`);
...
const newEngineCode = `function calculateMoveSynergyV33(learnedMovesMap, optName, tier, role, pkmnType) {
  ...
  // 5. 流派五：【弱點干擾流】(前置：叫聲、甩尾、挑釁、煙幕)
  var hasDebuffStance = learnedNames.some(function(n) { return /叫聲|甩尾|挑釁|煙幕/.test(n); });
  if (hasDebuffStance && (role === 'DIS' || /電磁波|劇毒|清除之煙|黑霧|滅亡之歌|咆哮/.test(optName))) {
...
if (oldEngineRegex.test(htmlContent)) {
  ...
  console.log('✅ 成功修復 learnedNames 解析並重新部署！');
} else {
  console.error('❌ 未找到 calculateMoveSynergyV33 函式');
}
