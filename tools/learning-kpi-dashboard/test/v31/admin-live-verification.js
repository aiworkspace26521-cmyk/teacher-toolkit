...
async function runAdminLiveVerification() {
  ...
  const phase1Events = [
    { action: '捕捉', note: 'ID:P1 獲得: 小火龍 (火系) | Lv.5 |', score: 100, timestamp: '2026-08-21T10:00:00.000Z' },
    { action: '捕捉', note: 'ID:P2 獲得: 小火馬 (火系) | Lv.5 |', score: 100, timestamp: '2026-08-21T10:05:00.000Z' }
  ];
  ...
  assert.ok(stateP1.roster.length >= 2, '應有至少 2 隻測試寶可夢 (P1 & P2)');
  
  const charmander = stateP1.roster.find(p => p.id === 'P1');
  const ponyta = stateP1.roster.find(p => p.id === 'P2');
  assert.ok(charmander, '應找到 P1 (小火龍)');
  assert.ok(ponyta, '應找到 P2 (小火馬)');
  
  console.log('    ✓ 管理員身份確認為 `Admin`');
  console.log('    ✓ 測試寶可夢初始化完成:', charmander.baseName, '與', ponyta.baseName);
  console.log('✅ 【階段 1】通過！\n');
  ...
  // 模擬管理員點擊 "+2000 EXP" 提升經驗值至滿級
  const phase2Events = [
    ...phase1Events,
    { action: 'B', note: 'ID:P1', score: 0, expGained: 200000, timestamp: '2026-08-21T10:10:00.000Z' }
  ];

  let stateP2 = await recalculateStudentState(adminStudentId, phase2Events);
  const p2Pkmn = stateP2.roster.find(p => p.id === 'P1');
  ...
  const testTiers = [
    { deltaSp: 3, targetTotal: 3, expectedTier: 2 },
    { deltaSp: 5, targetTotal: 8, expectedTier: 3 },
    { deltaSp: 7, targetTotal: 15, expectedTier: 4 },
    { deltaSp: 9, targetTotal: 24, expectedTier: 5 }
  ];
  ...
  for (const tt of testTiers) {
    accumEvents.push({
      action: 'SP_ALLOCATE',
      note: `${p2Pkmn.id}:atk:${tt.deltaSp}:v31`,
      score: 0,
      timestamp: '2026-08-21T10:15:00.000Z'
    });
    let st = await recalculateStudentState(adminStudentId, accumEvents);
    let targetAtkTree = st.roster.find(p => p.id === p2Pkmn.id).skillTree.atk;
    console.log(`    - 投入 ATK SP 累計達 ${targetAtkTree.sp} 點 -> 解鎖階層: T${targetAtkTree.tier}`);
    assert.strictEqual(targetAtkTree.sp, tt.targetTotal, `累計 SP 應為 ${tt.targetTotal}`);
    assert.strictEqual(targetAtkTree.tier, tt.expectedTier, `SP 達 ${tt.targetTotal} 應精確解鎖 T${tt.expectedTier}`);
  }
  ...
  const p3Pkmn = stateP3.roster.find(p => p.id === p2Pkmn.id);
  ...
  const p4Pkmn = stateP4.roster.find(p => p.id === p2Pkmn.id);
  ...
  for (let loop = 1; loop <= 3; loop++) {
    ...
    let rbPkmn = rollbackSt.roster.find(p => p.id === p2Pkmn.id);
    ...
    let actPkmn = activeSt.roster.find(p => p.id === p2Pkmn.id);
