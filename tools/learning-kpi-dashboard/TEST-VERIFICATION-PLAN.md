...
| U5-2 | 隊伍選擇 UI（1~3 隻彈性範圍） | Playwright | 可選 1~3 隻 |
...
> **2026-06-24 更新**：U7-1 預期值由 +2 更正為 +1。原因：Phase 4 Step A（好感度不一致修復）後續將 submit 親密度改回 +1，並統一 submit/capture/A 共用同一 +1 區塊。U7-2 捕捉 +1 維持不變。

| # | 驗證項目 | 方法 | 預期 |
|:-:|---------|------|------|
| U7-1 | 每日提交後全隊親密度 +1 | Playwright | 數值正確 |
...
**2026-06-24 驗證結果**：B-1~B-12 全數通過 ✅。靜態檢查: B-1(streakBonus 3/5/7累進)✅ B-2(60-74 2%基礎)✅ B-3(模擬鏡像)✅。邊界測試全部通過 ✅。模擬 ✅(2黃旗) E2E 74/77 ✅(2 pre-existing TM fail + 1 skip)。⚠️ B-11 minor 建議：§12.3 表缺少 75-94(5%)、60-74(2%) 分層與 streak 階梯表。
