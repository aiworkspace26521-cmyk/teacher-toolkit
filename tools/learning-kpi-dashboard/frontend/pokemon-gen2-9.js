...
// ========== EVO_CHAIN_MAP (mirror backend kpi-core.js) ==========
// 還有後續進化的寶可夢名單。判斷「最終型」＝該寶可夢不在本清單中（且 evoStage>=1）。
// 與後端 kpi-core.js 的 EVO_CHAIN_MAP 保持一致，避免 Gen2-9 進化型被誤判卡 T5。
var EVO_CHAIN_MAP = {
...
if (typeof window !== "undefined") window.EVO_CHAIN_MAP = EVO_CHAIN_MAP;
// 由 EVO_CHAIN_MAP 推導的「還有後續進化」完整集合：
// 涵蓋 chain 的 key（起點）與所有非終點的中間進化型（如 妙蛙草→妙蛙花、卡咪龜→水箭龜），
// 讓「最終型＝沒有後續進化」的判斷完整一致，不會因中間型漏列而誤開 T5。
var EVO_HAS_FURTHER = (function(){
  var s = {};
  if (typeof EVO_CHAIN_MAP === "undefined") return s;
  for (var k in EVO_CHAIN_MAP) {
    s[k] = true;
    var arr = EVO_CHAIN_MAP[k];
    if (arr && arr.length >= 2) {
      for (var i = 0; i < arr.length - 1; i++) s[arr[i]] = true;
    }
  }
  return s;
})();
if (typeof window !== "undefined") window.EVO_HAS_FURTHER = EVO_HAS_FURTHER;
